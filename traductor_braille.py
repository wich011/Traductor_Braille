...
def main(image_path):
    ...
    # Añade esta validación:
                if row >= 3:
                    row = 2  # Limita a la última fila (índice 2)
                if col >= 2:
                    col = 1   # Limita a la última columna (índice 1)
                
                if 0 <= row < 3 and 0 <= col < 2:  # Asegura que los índices estén en rango
                    grid[row][col] = 1
                else:
                    print(f"Punto fuera de rango: ({row}, {col})")  # Depuración
    ...
    print("Texto español:", texto_traducido)


if __name__ == '__main__':
    image_path = sys.argv[1] #se obtiene la ruta de la imagen desde los argumentos
    main(image_path)
